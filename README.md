# The Queen's Revel
### A Celtic Fae LARP

Every year at summer's end, the rulers of Britain's three great courts present themselves at the City of Light for the Queen's Revel. This year, Morgan has sealed herself into the Hawthorn Tower and refused to come out. Her three Knights host the Revel in her stead. Among the guests is Oberon, Sovereign of the Autumn Forest, who is not what he appears to be. A one-evening live-action drama for 8–15 players, approximately 4 hours.

---

## Contents
- [For Players](#for-players)
- [For Named Characters](#for-named-characters)
- [For Court Members (Unnamed)](#for-court-members-unnamed)
- [For Storytellers](#for-storytellers)
- [World and Rules Reference](#world-and-rules-reference)

---

## For Players

These files are safe to share with all players before session zero.

- [Player Primer](players/primer.md) — the world, the courts, and tonight's situation; read this before session zero
- [How to Play](players/how-to-play.md) — stats, tokens, conflict resolution, abilities, hospitality, the reveal
- [Character Sheet](players/character-sheet.md) — fillable form; print one per player

---

## For Named Characters

Each file below is a private brief for one pre-cast role. **Give each player only their own file.**

| Character | Faction | Brief |
|---|---|---|
| Oberon | Autumn Forest | [oberon.md](characters/named/oberon.md) — **read privately; do not share** |
| Aurora | Court of Storms | [aurora.md](characters/named/aurora.md) |
| Woodwose | Court of Beasts | [woodwose.md](characters/named/woodwose.md) |
| Mab | Court of Mirrors | [mab.md](characters/named/mab.md) |
| Barghest | Queen's Court | [barghest.md](characters/named/barghest.md) |
| Mélusine | Queen's Court | [melusine.md](characters/named/melusine.md) |
| Baobhan Sith | Queen's Court | [baobhan-sith.md](characters/named/baobhan-sith.md) |

---

## For Court Members (Unnamed)

Court members choose their own fae name, allocate their stats, pick abilities, and draw one Secret Mission card and one Secret Knowledge entry at session zero.

| Court | Sidhe | Sprite |
|---|---|---|
| Storms | [storm-sidhe.md](characters/templates/storm-sidhe.md) | [storm-sprite.md](characters/templates/storm-sprite.md) |
| Beasts | [beast-sidhe.md](characters/templates/beast-sidhe.md) | [beast-sprite.md](characters/templates/beast-sprite.md) |
| Mirrors | [mirror-sidhe.md](characters/templates/mirror-sidhe.md) | [mirror-sprite.md](characters/templates/mirror-sprite.md) |

- [Secret Mission Cards](characters/secret-missions.md) — 25 cards for Sidhe and Sprites to draw from
- [Secret Knowledge](characters/secret-knowledge.md) — one entry per player, distributed to all attendees including named characters at session zero

---

## For Storytellers

Files for running the event. Keep these with the storyteller packet.

- [Story Overview](storyteller/overview.md) — cast distribution, five-act structure with beats and timing
- [The Vortigern Reveal](storyteller/vortigern-reveal.md) — mechanics, trigger options, and stagecraft
- [Goals and Faction Storylines](storyteller/goals-and-factions.md) — cross-plot overview for each named character
- [Session Zero Guide](storyteller/session-zero.md) — full prep checklist and mixer overview
- [Mixer Facilitator Script](storyteller/mixer-script.md) — step-by-step runtime script for session zero
- [Storyteller Notes](storyteller/storyteller-notes.md) — court sizes, hidden heir setup, cross-plot interactions, re-draw guidance
- [Optional Add-Ons](storyteller/optional-addons.md) — fourth court, iron tokens, sequel hook

---

## World and Rules Reference

Background reading for the setting and mechanics.

- [Concept and World](world/concept.md) — the premise, Morgan's withdrawal, tonight's situation
- [The Courts](world/courts.md) — all five factions described
- [Rank and Stats](rules/rank-and-stats.md) — Sovereign/Sidhe/Sprite tiers, point allocations, nine stats in three categories (Iron/Court/Wyrd)
- [Abilities](rules/abilities.md) — five universal disciplines (Imbas, Geis, Veil, Awen, The Green) plus per-court abilities, tier costs, and purchasing rules
- [Named Character Abilities](rules/named-abilities.md) — Knight signatures and Oberon's ability sheets (storyteller reference)
- [Conflict Resolution](rules/conflict-resolution.md) — RPS throw, stat tie-breakers, re-test procedure
- [Hospitality Rules](rules/hospitality.md) — what is and isn't permitted until Act IV
